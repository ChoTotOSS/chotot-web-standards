...
function memberExpressionSelector(options) {
  ...
  let { path, property, properties, object, objects, includeOptional, allowComputed } = {
    path: '',
    property: '',
    properties: [],
    object: '',
  ...
  const parts = [`[${prefix}type="MemberExpression"]`];

  if (!allowComputed) {
    parts.push(`[${prefix}computed!=true]`, `[${prefix}property.type="Identifier"]`);
  }
